...
local function is_within_maiden_circle(position, maiden_pos, radius)
    if not position or not maiden_pos then return false end
    local distance = position:dist_to(maiden_pos)
...
local stateFunctions = {
    [States.IDLE] = function(menu_elements, helltide_final_maidenpos, explorer_circle_radius)
        ...
        -- Verificar se o player está dentro do círculo
        local player_pos = get_player_position()
        if not is_within_maiden_circle(player_pos, helltide_final_maidenpos, explorer_circle_radius) then
        ...
        if current_hearts > 0 then
            ...
            for _, actor in ipairs(actors) do
                ...
                -- Debug: imprimir todos os atores encontrados
                console.print("Actor encontrado: " .. name .. " em " .. get_position_string(actor_pos))
                
                if name == "S04_SMP_Succuboss_Altar_A_Dyn" then
                    if is_within_maiden_circle(actor_pos, helltide_final_maidenpos, explorer_circle_radius) then
                        if not is_blacklisted(actor) then
                            table.insert(targetAltars, actor)
                            console.print("Altar válido adicionado: " .. get_position_string(actor_pos))
                        else
                            console.print("Altar na blacklist ignorado: " .. get_position_string(actor_pos))
                        end
                    else
                        console.print("Altar fora do círculo ignorado: " .. get_position_string(actor_pos))
                    end
                end
            end
    ...
    [States.MOVING] = function(menu_elements, helltide_final_maidenpos, explorer_circle_radius)
        ...
        if not is_within_maiden_circle(altar_pos, helltide_final_maidenpos, explorer_circle_radius) then
            console.print("Altar saiu do círculo da maiden, ignorando")
